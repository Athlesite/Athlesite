...
export function EditorialHome() {
  return <HomeMotion className={s.home}>
    ...
    <section id="for-athletes" className={`${s.section} ${s.productSection}`} aria-labelledby="product-title">
      <div className={s.wrap}>
        ...
        <div className={s.productGrid}>
          <Reveal variant="image" className={s.productVisual}><span className={s.visualLabel}>A CLOSER LOOK</span><ProfilePreview device="laptop" /><div className={s.previewCaption}><span>Jordan Bell / Fictional example</span><span>Desktop + mobile</span></div></Reveal>
          <Reveal variant="right" delay={90} className={s.productCopy}>
    ...
    <section id="recruiting" className={s.section} aria-labelledby="identity-title">
      <div className={s.wrap}>
        ...
        <div className={s.featureGrid}>
          <Reveal variant="image" className={s.editorialPhoto}><Image src="/marketing/editorial/football-team.jpg" alt="Football team lined up together on the field" fill sizes="(max-width: 800px) 100vw, 50vw" /><div className={s.photoCaption}><span>FOR THE MOMENTS THAT MATTER.</span><span>ON AND OFF THE FIELD.</span></div></Reveal>
          <Reveal variant="right" delay={90}><FeatureExplorer /><div className={s.featureFootnote}><span aria-hidden="true">↗</span><p>A clear introduction.<br />A better starting point for what comes next.</p></div></Reveal>
        </div>
      </div>
    </section>
    ...
    <section className={`${s.section} ${s.storySection}`} aria-labelledby="story-title">
      <div className={s.wrap}>
        ...
        <div className={s.storyGrid}>
          <Reveal variant="image" className={s.storyCard}><div className={s.storyImage}><Image src="/marketing/editorial/football-detail.jpg" alt="A well-worn football helmet carried beside a grass field" fill sizes="(max-width: 700px) 100vw, 33vw" /></div><div className={s.storyCardText}><span>01 / THE WORK</span><h3>What gets you here.</h3><p>The training, discipline, and details behind your game.</p></div></Reveal>
          <Reveal variant="image" delay={90} className={s.storyCard}><div className={s.storyImage}><Image src="/marketing/editorial/football-night.jpg" alt="Football player warming up on the field" fill sizes="(max-width: 700px) 100vw, 33vw" /></div><div className={s.storyCardText}><span>02 / THE GAME</span><h3>What you bring.</h3><p>Your best moments, with the story to go with them.</p></div></Reveal>
          <Reveal delay={180} className={`${s.storyCard} ${s.storyStatement}`}><span className={s.cornerArrow} aria-hidden="true">↗</span><div><span>03 / WHAT’S NEXT</span><h3>Not just<br />where<br />you play.<br /><em>Where<br />you’re going.</em></h3><p>A home for your ambition—on and beyond the field.</p></div></Reveal>
        </div>
      </div>
    </section>
    ...
    <section className={`${s.section} ${s.startSection}`} aria-labelledby="start-title"><div className={s.wrap}>
      ...
      ].map(([number, title, copy], index) => <Reveal delay={index * 90} key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p></Reveal>)}</div>
    </div></section>
